...
EXERCISE 5: Turn the previous code into a function which returns the dataframe with RTs and responses

```{r}
simulate_DDM <- function(v = 3, a = 1, z = 0, ter = .3,
                        s = 1, dt = .001, tmax = 10, ntrials = 10){
...
fptd_data2 <- simulate_DDM(v = 2, n = 2000)
...
d <- DDM() %>%
  filter(resp == 1)
qs <- quantile(d$rt, probs = c(.1,.3,.5,.7,.9))
plot(density(d$rt), main = "")
abline(v = qs)
```
...
accuracy_data <- example_data %>%
  filter(instruction == "accuracy",
         id == 'jf') %>% 
  select(rt,resp)
optimal_parameters_accuracy <- DEoptim(loss_gsquare, 
                              lower = c(0, .2, 0), upper = c(3, 2, .5), 
                              observed_data = accuracy_data, 
                              ntrials = 10000,
                              control=list(itermax=100,trace=10))
predicted_data <- DDM(v = optimal_parameters_accuracy$optim$bestmem[1],
                      a = optimal_parameters_accuracy$optim$bestmem[2],
                      ter = optimal_parameters_accuracy$optim$bestmem[3],
                      ntrials = 100000)
plot_fptd(data = accuracy_data, comparison_data = predicted_data)
```
...
speed_data <- example_data %>% 
  filter(instruction == "speed",
         id == 'jf') %>% 
  select(rt,resp)
optimal_parameters_speed <- DEoptim(loss_gsquare, 
                              lower = c(0, .2, 0), upper = c(3, 2, .5), 
                              observed_data = speed_data, 
                              ntrials = 10000,
                              control=list(itermax=100,trace=10))
predicted_data <- DDM(v = optimal_parameters_speed$optim$bestmem[1],
                      a = optimal_parameters_speed$optim$bestmem[2],
                      ter = optimal_parameters_speed$optim$bestmem[3],
                      ntrials = 100000)
plot_fptd(data = speed_data, comparison_data = predicted_data)
```
...
loss_ks_ster <- function(parms, observed_data, ntrials = 5000){
  ...
  observed_data <- flip_errors(observed_data)
  # generate data, given the candidate parameters in the parms vector a flip the errors
  predicted_data <- DDM(v = parms[1], a = parms[2], ter = parms[3], ster = parms[4], ntrials=ntrials)
  predicted_data <- flip_errors(predicted_data)
  # compute the KS statistic
  ks <- suppressWarnings(ks.test(observed_data$rt, predicted_data$rt))
  return(ks$statistic)
...
speed_data <- example_data %>% 
  filter(instruction == "speed",
         id == 'jf') %>% 
  select(rt,resp)
optimal_parameters_speed_ster <- DEoptim(loss_ks_ster, 
                              lower = c(0, .2, 0, 0), upper = c(3, 2, .5, .5), 
                              observed_data = speed_data, 
                              ntrials = 10000,
                              control=list(itermax=100,trace=10))
predicted_data <- DDM(v = optimal_parameters_speed_ster$optim$bestmem[1],
                      a = optimal_parameters_speed_ster$optim$bestmem[2],
                      ter = optimal_parameters_speed_ster$optim$bestmem[3],
                      ster = optimal_parameters_speed_ster$optim$bestmem[4],
                      ntrials = 100000)
plot_fptd(data = speed_data, comparison_data = predicted_data)
```
